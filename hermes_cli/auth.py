...
def _load_opencode_acp_config() -> Dict[str, Any]:
    ...
    if not isinstance(cfg, dict):
        return {}
    # ``opencode_acp`` is the canonical key; ``wsl_opencode_acp`` is the
    # historical local-patch key kept for compatibility with Rodrigo's host.
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        if provider_id == "opencode-acp":
            message = (
                ...
                "Install OpenCode CLI or set HERMES_OPENCODE_ACP_COMMAND/OPENCODE_CLI_PATH."
            )
    ...
    return {
        "provider": provider_id,
        "api_key": str(runtime.get("api_key") or provider_id),
        "base_url": base_url,
        "command": resolved_command or command,
        "args": list(runtime.get("args") or []),
        "source": str(runtime.get("source") or "process"),
    }
